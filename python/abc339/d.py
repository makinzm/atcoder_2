...
n = int(input())
s_list = []
visited_matrix = [[False for _ in range(n**2)] for _ in range(n**2)]
...
queue = deque([(P_x * n + P_y, Q_x * n + Q_y ,0)])
ans_count = 2 ** 31 - 1

while len(queue) > 0:
    P_x_P_y, Q_x_Q_y, count = queue.popleft()
    P_x = P_x_P_y // n
    P_y = P_x_P_y % n
    Q_x = Q_x_Q_y // n
    Q_y = Q_x_Q_y % n
    ...
    if count >= ans_count:
        continue
    visited_matrix[P_x_P_y][Q_x_Q_y] = True
    for i in range(4):
        ...
        if new_P_x == new_Q_x and new_P_y == new_Q_y:
            ...
            if not visited_matrix[new_P_x * n + new_P_y][new_Q_x * n + new_Q_y]:
                queue.append((new_P_x * n + new_P_y, new_Q_x * n + new_Q_y, count))
